...
const describeFeed = `-- name: DescribeFeed :one
SELECT id, created_at, updated_at, name, url, user_id, last_fetched_at FROM feeds
WHERE url = $1
`

func (q *Queries) DescribeFeed(ctx context.Context, url string) (Feed, error) {
	row := q.db.QueryRowContext(ctx, describeFeed, url)
	var i Feed
	err := row.Scan(
		&i.ID,
		&i.CreatedAt,
